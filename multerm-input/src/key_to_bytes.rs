...
pub fn key_to_bytes(event: &KeyEvent, mods: ModifiersState, app_cursor: bool) -> Option<Vec<u8>> {
    ...
    match &event.logical_key {
        Key::Named(named) => Some(named_key_bytes(*named, mods, app_cursor)),
        Key::Character(s) => {
    ...
    match key {
        ...
        NamedKey::Home => {
            if mod_suffix.is_empty() {
                ...
                format!("\x1b[1{}H", mod_suffix).into_bytes()
            }
        }
        NamedKey::End => {
            if mod_suffix.is_empty() {
                ...
                format!("\x1b[1{}F", mod_suffix).into_bytes()
            }
        }
        NamedKey::PageUp => {
            if mod_suffix.is_empty() {
                ...
                format!("\x1b[5{}~", mod_suffix).into_bytes()
            }
        }
        NamedKey::PageDown => {
            if mod_suffix.is_empty() {
                ...
                format!("\x1b[6{}~", mod_suffix).into_bytes()
            }
        }
        NamedKey::Insert => {
            if mod_suffix.is_empty() {
                ...
                format!("\x1b[2{}~", mod_suffix).into_bytes()
            }
        }
        NamedKey::Delete => {
            if mod_suffix.is_empty() {
                ...
                format!("\x1b[3{}~", mod_suffix).into_bytes()
            }
        }
