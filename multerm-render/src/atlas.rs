...
use cosmic_text::{Attrs, Buffer, Color as CosmicColor, Family, FontSystem, Metrics, Shaping, Style, SwashCache, Weight};
...
impl GlyphAtlas {
    ...
    pub fn set_raster_scale(&mut self, scale: f32) {
        if (scale - self.raster_scale).abs() > 0.001 {
            ...
            self.cache.clear();
            self.allocator = AtlasAllocator::new(
                Size::new(self.atlas_size as i32, self.atlas_size as i32)
            );
        }
    }
    ...
    fn rasterize(&mut self, gpu: &GpuContext, key: GlyphKey) -> Option<GlyphUV> {
        ...
        buf.set_size(&mut self.font_system, Some(cell_w as f32), Some(cell_h as f32));
        ...
        buf.draw(&mut self.font_system, &mut self.swash_cache,
        ...
        let alloc = match self.allocator.allocate(Size::new(cell_w as i32, cell_h as i32)) {
            Some(a) => a,
            None => {
                ...
                self.allocator.allocate(Size::new(cell_w as i32, cell_h as i32))?
            }
        };
        ...
        gpu.queue.write_texture(
            ...
            wgpu::Extent3d { width: cell_w, height: cell_h, depth_or_array_layers: 1 },
        );
