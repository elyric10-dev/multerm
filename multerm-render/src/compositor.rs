...
use multerm_vt::{cell::WideKind, TerminalGrid};
...
impl Compositor {
    pub fn new(gpu: &GpuContext, atlas: &GlyphAtlas) -> anyhow::Result<Self> {
        ...
        let atlas_bg =
            Self::make_atlas_bind_group(device, &atlas_bg_layout, &atlas.texture_view, &sampler);
        ...
        let glyph_buf =
            Self::make_instance_buf::<GlyphInstance>(device, GLYPH_INIT_CAP, "glyph_instances");
    ...
    pub fn render_terminal_panes(
        ...
        gpu.queue
            .write_buffer(&self.uniform_buf, 0, cast_slice(&uniform_data));
        ...
        for (pane_idx, (rect_px, grid)) in panes.iter().enumerate() {
            ...
            for row in 0..grid.rows {
                for col in 0..grid.cols {
                    ...
                    let is_cursor_cell = cursor.is_some_and(|cursor_state| {
                        cursor_state.row == row && cursor_state.col == col
                    });

                    let px = origin_x + col as f32 * cell_w;
                    let py = origin_y + row as f32 * cell_h;
                    let gw = if cell.wide == WideKind::Leading {
                        cell_w * 2.0
                    } else {
                        cell_w
                    };
        ...
        if !bg_insts.is_empty() {
            gpu.queue
                .write_buffer(&self.bg_buf, 0, cast_slice(&bg_insts));
        }
        if !glyph_insts.is_empty() {
            gpu.queue
                .write_buffer(&self.glyph_buf, 0, cast_slice(&glyph_insts));
        }
    ...
    fn ensure_bg_buf(&mut self, gpu: &GpuContext, need: usize) {
        if need > self.bg_buf_cap {
            ...
            self.bg_buf =
                Self::make_instance_buf::<BgQuadInstance>(&gpu.device, new_cap, "bg_instances");
    ...
    fn ensure_glyph_buf(&mut self, gpu: &GpuContext, need: usize) {
        if need > self.glyph_buf_cap {
            ...
            self.glyph_buf =
                Self::make_instance_buf::<GlyphInstance>(&gpu.device, new_cap, "glyph_instances");
